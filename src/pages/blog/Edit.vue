    ...
    <el-form :model="blog" :rules="rules" ref="blog" label-width="100px" class="demo-blog">
      ...
      <el-form-item label="分类" prop="category_id">
        <el-select v-model="blog.category_id" placeholder="请选择文章分类">
          <el-option v-for="item in categories" :label="item.v" :value="item.k" v-if="" :key="index"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item v-if="tags != null" label="标签" prop="tags">
        <el-checkbox-group v-model="blog.tags">
          <el-checkbox v-for="item in tags" :label="item.k" :key="index">{{item.v}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
  ...
  export default {
    ...
    data() {
      return {
        ...
        uploadDomain: 'https://up-z0.qiniup.com',
        // 这是七牛云空间的外链默认域名
        qiniuAddr: 'img.dabin-coder.cn',
        blog: {
          title: '',
          category_id: null,
          code: 0,
    ...
    methods: {
      ...
      noticeMessage(code, msg, isJumTo) {
        if (!this.isEmpty(code) && code === 1000) {
          if (isJumTo) {
            ...
            this.$router.push({path: '/blogs/category/0'})
          } else {
      ...
      handleImgAdd(pos, uploadFile) {
        ...
        this.uploadFileToQiniu(pos, uploadFile)
      },
      // 上传文件到七牛云
      uploadFileToQiniu(pos, file) {
        const config = {
        ...
        // 重命名要上传的文件
        var date = new Date()
        const newFileName = 'dabin' + date.getTime() + '.' + fileType
        // 从后端获取上传凭证token
